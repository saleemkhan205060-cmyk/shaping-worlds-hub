...
-- Phase 2: Enhanced link protection. Extends classify_risky_text only;
-- existing posts/comments/profiles triggers continue calling it.
...
BEGIN
  ...
  -- Phase 2: URL scanning. Pull first http(s) or bare-domain URL.
  url_match := (regexp_match(s, '(https?://[^\s<>")]+|(?:^|[\s])((?:[a-z0-9-]+\.)+[a-z]{2,})(/[^\s<>")]*)?'))[1];
  ...
  IF url_match IS NOT NULL THEN
    ...
    IF host <> '' THEN
      -- known bad shorteners / messenger links
      ...
      -- IP-literal URLs (e.g. http://203.0.113.5/foo)
      IF host ~ '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$' THEN
        RETURN 'suspicious_link';
      END IF;

      -- trusted allowlist (exact or subdomain of)
      ...
      -- anything else: hold for admin review
