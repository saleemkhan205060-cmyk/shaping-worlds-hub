...
import { Pencil, Save, X, ChevronDown, User, UserPlus, Share2, Star } from "lucide-react";
          ...
          <div data-no-translate className="px-4 pb-6 overflow-y-auto text-sm text-slate-700 space-y-3">
            <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              <div className="flex items-center gap-2">
                {about.isPublic ? <Globe className="h-4 w-4 text-emerald-600" /> : <Lock className="h-4 w-4 text-slate-600" />}
                <div>
                  <div className="text-xs font-semibold text-slate-900">
                    {about.isPublic ? "Public" : "Private"}
                  </div>
                  <div className="text-[10px] text-slate-500">
                    {about.isPublic ? "Sab users dekh sakte hain" : "Sirf aap dekh sakte hain"}
                  </div>
                </div>
              </div>
              <div className="inline-flex rounded-full bg-white border border-slate-200 p-0.5">
                <button
                  type="button"
                  onClick={async () => {
                    if (!user || about.isPublic) return;
                    const { error } = await supabase.from("profile_about").upsert({ user_id: user.id, is_public: true });
                    if (error) { toast.error("Failed to update"); return; }
                    setAbout({ ...about, isPublic: true });
                    toast.success("About is now Public");
                  }}
                  className={`px-3 py-1 rounded-full text-[11px] font-semibold inline-flex items-center gap-1 ${about.isPublic ? "bg-emerald-600 text-white" : "text-slate-600"}`}
                >
                ...
                <button
                  ...
                  onClick={async () => {
                    if (!user || !about.isPublic) return;
                    const { error } = await supabase.from("profile_about").upsert({ user_id: user.id, is_public: false });
                    if (error) { toast.error("Failed to update"); return; }
                    setAbout({ ...about, isPublic: false });
                    toast.success("About is now Private");
                  }}
                  className={`px-3 py-1 rounded-full text-[11px] font-semibold inline-flex items-center gap-1 ${!about.isPublic ? "bg-slate-700 text-white" : "text-slate-600"}`}
                >
                  <Lock className="h-3 w-3" /> Private
                </button>
              </div>
