...
export function FullscreenVideoPlayer({ items, startIndex, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [likesRes, { data: comments }] = await Promise.all([
        user
      ...
      ((likes ?? []) as unknown as (PostLikeRow & { like_count?: number })[]).forEach((l) => {
        if (!user) {
...
setTimeout(() => {
  ...
  return (
    <div className="fixed inset-x-0 top-0 bottom-[68px] z-[20] bg-black">
      <div
        ref={containerRef}
...
>
  ...
  <button
    ...
    className="absolute right-0 bottom-0 z-50 h-5 w-5 rounded-full bg-[#117D43] text-white flex items-center justify-center shadow-md border border-white active:scale-90"
    aria-label="Follow"
    >
      ...
      {shareItem && (
        <ShareSheet
          ...
          url={shareItem.user_id ? buildShareUrl(`/u/${shareItem.user_id}`) : buildShareUrl("/")}
        />
      )}
      {thumbPickFor && (
