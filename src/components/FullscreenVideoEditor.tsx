...
import { MoreVertical, Upload, Play, Pause, X, Sparkles, Music, Scissors, Volume2, VolumeX, Crop, SlidersHorizontal, Gauge, Type, Pencil, Camera, Check, RotateCcw } from "lucide-react";
...
const ASPECTS: { id: string; label: string; ratio: string }[] = [
  { id: "custom", label: "Custom", ratio: "auto" },
    ...
    <div className="fixed inset-0 z-[400] bg-black flex flex-col">
      ...
      {sheet === "edit" && (
        <div className="fixed inset-0 z-[420] bg-black flex flex-col" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex-1 min-h-0 flex items-center justify-center relative px-6 py-4">
            <div className="relative max-h-full max-w-full" style={aspect === "custom" ? {} : aspectStyle}>
              {src && (
                <video
                  ref={editPreviewRef}
                  src={src}
                  className={aspect === "custom" ? "max-h-full max-w-full object-contain" : "w-full h-full object-contain"}
                  style={{ filter: videoFilter, ...(aspect === "custom" ? {} : aspectStyle) }}
                  muted
                  playsInline
                  onClick={togglePlay}
              ...
              {/* Corner brackets (Crop tab only) */}
              {editTab === "crop" && (
                <>
                  <span className="pointer-events-none absolute -top-2 -left-2 w-7 h-7 border-t-[3px] border-l-[3px] border-white rounded-tl-xl" />
                  <span className="pointer-events-none absolute -top-2 -right-2 w-7 h-7 border-t-[3px] border-r-[3px] border-white rounded-tr-xl" />
                  <span className="pointer-events-none absolute -bottom-2 -left-2 w-7 h-7 border-b-[3px] border-l-[3px] border-white rounded-bl-xl" />
                  <span className="pointer-events-none absolute -bottom-2 -right-2 w-7 h-7 border-b-[3px] border-r-[3px] border-white rounded-br-xl" />
                  {/* Center play + timestamp */}
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="flex items-center gap-3 text-white">
                      <Play className="h-6 w-6 fill-white" />
                      <span className="text-base font-medium tabular-nums">{fmt(current)} / {fmt(duration)}</span>
                    </div>
                  </div>
                </>
              )}
          ...
          {editTab === "crop" ? (
            <>
                ...
                {ASPECTS.map((a) => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 w-[70px] h-[70px] rounded-2xl flex flex-col items-center justify-center gap-1 transition active:scale-95 ${active ? "bg-slate-800 ring-2 ring-indigo-400" : "bg-slate-800/70"}`}
                    >
                      <div
                        className="bg-slate-300 rounded-[3px]"
                        style={{
                          width: a.id === "custom" ? 24 : a.id === "square" ? 22 : 18,
                          height: a.id === "custom" ? 18 : a.id === "square" ? 22 : 26,
                        }}
                      />
              ...
              {/* Bottom action pill: X | crop+rotate | check */}
              <div className="flex items-center justify-between px-6 pb-5 shrink-0">
                <button
                  onClick={onClose}
                  className="h-12 w-12 rounded-full bg-slate-800/80 text-white flex items-center justify-center active:scale-95"
                  aria-label="Cancel"
                >
                  <X className="h-5 w-5" />
                </button>
                <div className="flex items-center bg-slate-800/80 rounded-full p-1.5 gap-1">
                  <button
                    className="h-11 w-11 rounded-full bg-indigo-500 text-white flex items-center justify-center active:scale-95"
                    aria-label="Crop"
                  >
                  ...
                  <button
                    ...
                    className="h-11 w-11 rounded-full text-white flex items-center justify-center active:scale-95"
                    aria-label="Rotate"
                  >
            ...
            <>
              ...
              {/* Trim strip with draggable handles */}
